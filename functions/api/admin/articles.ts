...
export async function onRequest(context: any) {
  try {
    ...
    if (method === 'GET') {
      ...
      return new Response(JSON.stringify({ articles: data }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }

    const body = await context.request.json().catch(() => ({}));

    if (method === 'POST') {
      const { title, slug, excerpt, content, author, category, tags, is_published } = body;
      const payload: any = { title, slug, excerpt, content, author, category, tags, is_published };
      const { data, error } = await supabase.from('articles').insert([payload]).select();
      if (error) throw error;
      return new Response(JSON.stringify({ article: data?.[0] || null }), { status: 201, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (method === 'PUT') {
      ...
      if (error) throw error;
      return new Response(JSON.stringify({ article: data?.[0] || null }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
